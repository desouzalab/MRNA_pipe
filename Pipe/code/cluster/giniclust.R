...
for (c in 1:length(all_preprocessed_ssRNASeq_files)){
  ...
  GeneList.final = GiniClust_Fitting(data.type = 'RNA-seq',ExprM.RawCounts.filter=data,out.folder=paste0(data_outdir,"/extra"),exprimentID=c)
  print("done fitting")
  ...
  write.csv(clustering_membership_r, file=paste0(data_outdir,"/giniclust_",args$name_dataset,".csv"))
  rm(list = ls())
