...
for (c in 1:length(all_raw_ssRNASeq_files)){
  ...
  seurat_object <- CreateSeuratObject(counts = data, project = "data_cutoff", min.cells = 3, min.features = 100)
  seurat_object
  ...
  seurat_object <- subset(seurat_object, subset = nFeature_RNA>1700 & nFeature_RNA <3600 & nCount_RNA>49700)
  #Normalization
